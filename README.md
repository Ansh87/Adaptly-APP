# SATGene — Adaptive SAT Preparation Agent
...
- **Today's Mission** — 2–4 bite-sized actions pulled from the same priorities.
...
richer study plan narrative, and the "Explain differently" tutor step — and both have a
...
| **My Results** | Log official SAT scores, practice scores, and mistakes — the evidence the agent reasons over. Section scores are validated to 200–800; totals are always derived. |
...
- **No paywall.** Practice content is either original (SATGene's own question bank) or
  links straight to free official resources (Bluebook, Student Question Bank, Khan
  Academy) — never a proprietary content lock-in.
- **No signup required to try it.** Demo Student mode gives any student (or judge) the
  full experience with realistic sample data in one click.
