...
/**
 * Adaptly — Digital SAT Practice, Analytics & Study Planner
 * -----------------------------------------------------------------
 ...
 * So Adaptly does NOT reinvent the test. It is the HUB + TRACKER
...
const DEFAULT_PLANS = [];
// Adaptly Agent state (Phase 1). practiceEvents are individual adaptive-practice
...
// the live agent output against this baseline is what powers "Adaptly Noticed"
// and "Why My Plan Changed."
...
// buildAgentSnapshot captures a lightweight
...
// a real account.
// ============================================================
const DEMO_USER = { uid: "demo-student", displayName: "Demo Student", email: "demo@adaptly.app (sample account)", photoURL: null };
...
// stored "Adaptly Noticed" / "Why My Plan Changed" baseline.
...
// ================================================================
export default function AdaptlyAI() {
    ...
    return (
      <div style={{ minHeight: "100vh", background: C.paper, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: FONT_BODY, color: C.ink2 }}>
        <div style={{ textAlign: "center" }}>
          <img src={logo} alt="Adaptly" style={{ height: 56, width: "auto" }} />
...
function AppShell({ user, demo = false, onExitDemo }) {
  ...
  // Recompute mastery whenever evidence changes (new test, new
  // mistake, or — from Phase 2 on — a new adaptive-practice result).
  // On the first pass after load it seeds from the saved
  ...
  // still be stale in this render pass) so "Adaptly Noticed" has something correct
  ...
  // ---- Auto-reassessment change tracking (Phase 3) ----
    ...
    <header style={{ borderBottom: `1px solid ${C.line}`, background: C.card }}>
      ...
      <div className="sg-container sg-header-main">
        <div className="sg-min0">
          <img src={logo} alt="Adaptly" className="sg-logo" style={{ display: "block" }} />
  ...
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
      ...
      <button
        ...
        aria-label="Settings"
        title="Settings"
        style={{ background: "none", border: "none", padding: 2, cursor: "pointer", color: C.ink2, lineHeight: 0, borderRadius: 8 }}
      >
        <IconGear />
      </button>
    </div>
...
// ---------- Adaptly Agent Dashboard (Phase 1) ----------
...
// ---- Active Study Plan (Home): a compact pointer into the AI Planner
// (the Planning tab in the primary nav).
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: 20 }}>
      ...
      <p style={{ fontSize: 13, color: C.ink2, margin: "0 0 14px", lineHeight: 1.5 }}>
        Adaptly automatically adjusts recommendations when new results are recorded.
      </p>
...
// ---- Adaptly Noticed / Why My Plan Changed (Phase 3) ----
// Both cards are pure renders of diffs computed in AppShell
// (computeAgentNotices / buildPlanChangeExplanation).
    ...
    <div role="status" style={{ background: C.card, border: `1px solid ${C.accent}`, borderRadius: 14, padding: 18, marginBottom: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, gap: 8, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <AgentBadge />
          <div style={{ fontWeight: 800, fontSize: 14.5, fontFamily: FONT_DISPLAY }}>Adaptly Noticed</div>
        </div>
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: 24 }}>
      ...
      <p style={{ color: C.ink2, fontSize: 14, lineHeight: 1.6, margin: "0 0 16px", maxWidth: 560 }}>
        ...
        Add either one and Adaptly will name your #1 priority skill with a data-grounded reason, no guessing, no fake numbers.
      </p>
...
function ResultsPage({ resultsTab, setResultsTab, attempts, setAttempts, mistakes, setMistakes }) {
  return (
    <>
      <SectionTitle kicker="Your evidence" title="My Results" sub="SAT scores, practice scores, and mistakes: the evidence Adaptly uses to understand your progress and decide what to work on next." />
...
function Mistakes({ mistakes, setMistakes, attempts }) {
  ...
  return (
    <>
      <p style={{ color: C.ink2, fontSize: 14, margin: "0 0 18px", maxWidth: 640 }}>Record why you missed each question, not the copyrighted question itself, just the skill and the lesson. Tag each as SAT or Practice so Adaptly can weigh them correctly.</p>
...
// ---------- 4. ANALYTICS ----------
// ============================================================
// ANALYTICS UTILITIES — testable helpers.
...
function Progress({ attempts, mistakes, goal, setTab, mastery, priorities }) {
  ...
  return (
    <>
      <SectionTitle kicker="Am I improving?" title="Progress" sub="Your My Results data turned into clear, actionable insights." />
...
function Planner({ attempts, mistakes, goal, setGoal, plans, setPlans, setTab, mastery, priorities, practiceEvents }) {
  ...
  return (
    <>
      <SectionTitle kicker="What to do next" title="AI Study Planner" sub="Separate plans for the official SAT and for your next practice test. Each plan is written by AI from your saved scores, mistakes, mastery, and priorities." />
...
function PlanPanel({ kind, latest, supportingLatest, target, onTarget, nextDate, onNextDate, attempts, mistakes, onAddPlan, setTab, mastery, priorities, practiceEvents }) {
  ...
  // Compute the factual numbers shown on a saved plan (scores, gap, weakest
  // section). The plan itself — summary, focus, weekly tasks, schedule, next
  // action — is authored entirely by the AI from the learner state below.
  const computeFacts = () => {
    ...
    return {
      ...
      targetScore: target,
      gap,
      rw: latest ? latest.rw : null,
      math: latest ? latest.math : null,
      weakestSection,
    };
  };
  ...
  const generate = async () => {
    ...
    setLoading(true);
    try {
      // Server-side Gemini calls take either a signed-in Adaptly user's ID
      // token (Fix 8) or the demo flag — Demo Student mode has no token but
      // still gets the full AI planner experience.
      const token = await getIdToken();
      const headers = { "Content-Type": "application/json" };
      if (token) headers.Authorization = `Bearer ${token}`;
      // Fix 7: give Gemini the same learner state the agent uses elsewhere
      // (mastery, top priorities, recent adaptive-practice results), so the
      // AI plan explains and expands the agent's ranking rather than
      // contradicting it.
      ...
      if (!res.ok) { const d = await res.json().catch(() => ({})); throw new Error(d.error || "The AI service is temporarily unavailable. Please try again in a moment."); }
      const data = await res.json();
      // Merge the AI-authored plan with the computed score facts so the saved
      // plan is complete.
      saveAndShow({ ...computeFacts(), ...data }, "AI");
    } catch (e) {
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: 20, marginBottom: 24 }}>
      ...
      <div className="sg-btn-row" style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        <button onClick={generate} style={btnPrimary} disabled={loading}>
  ...
  return (
    <>
      <SectionTitle
        ...
        title="Practice"
        sub="Complete Adaptly-recommended adaptive practice, run a full-length timed mock, or open trusted SAT resources."
      />
  ...
  return (
    <>
      <p style={{ color: C.ink2, fontSize: 14, margin: "0 0 16px", maxWidth: 640 }}>
        Official practice is free and the most realistic, so do it first. Paid vendors add extra question volume once you've exhausted the official pool. Each card opens the provider directly; Adaptly never copies their questions.
      </p>
...
// Adaptly-authored question bank. Difficulty adapts question-to-question, and
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: 22 }}>
      ...
      <p style={{ fontSize: 12, color: C.ink2, marginTop: 14 }}>
        Every question is original, written by Adaptly, never copied from College Board or any vendor. Difficulty adapts as you answer.
      </p>
    </div>
  );
}
...
function PracticeQuestion({ session, setSession, onPracticeResult, onRestart, onCompleteMissionItem }) {
  ...
  const explain = async () => {
    ...
    // Server-side Gemini calls take either a signed-in Adaptly user's ID token
    // (Fix 8) or the demo flag — Demo Student mode has no token but still gets
    // the AI tutor.
    const token = await getIdToken();
    const headers = { "Content-Type": "application/json" };
    if (token) headers.Authorization = `Bearer ${token}`;
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: 26, textAlign: "center" }}>
      ...
      <p style={{ fontSize: 13, color: C.ink2, maxWidth: 440, margin: "0 auto 20px" }}>
        Adaptly has updated your recommendations based on this practice.
      </p>
...
function MorePage({ user, displayName, syncState, profile, setProfile, goal, setGoal, attempts, mistakes, plans, setAttempts, setMistakes, setPlans, mastery, practiceEvents, missionCompleted, setMastery, setPracticeEvents, setMissionCompleted, agentSnapshot, setAgentSnapshot, demo, onExitDemo }) {
  ...
  return (
    <>
      <SectionTitle kicker="Account & information" title="Settings" sub="Manage your profile, preferences, saved data, and Adaptly information." />
...
function DataPrivacyPanel({ profile, goal, attempts, mistakes, plans, setGoal, setAttempts, setMistakes, setPlans, mastery, practiceEvents, missionCompleted, setMastery, setPracticeEvents, setMissionCompleted, agentSnapshot, setAgentSnapshot, demo, onExitDemo }) {
  ...
  const exportData = () => {
    ...
    a.href = url; a.download = `adaptly-backup-${new Date().toISOString().slice(0, 10)}.json`; a.click();
  ...
  const importData = (e) => {
    ...
    reader.onload = () => {
      try {
        ...
        setMsg({ kind: "err", text: "That file couldn't be read. Use a Adaptly backup file (.json)." });
      }
    };
    ...
    <>
      ...
      <PanelCard>
        ...
        <p style={{ fontSize: 13.5, color: C.ink2, lineHeight: 1.55, margin: 0 }}>
          Adaptly stores your profile, SAT and practice scores, mistake log, targets, test dates, and generated
      ...
      <div className="sg-grid">
        ...
        <PanelCard style={{ marginBottom: 0 }}>
          ...
          <p style={{ fontSize: 13, color: C.ink2, margin: "0 0 12px" }}>Load a Adaptly backup file (replaces current data).</p>
...
function HelpPanel() {
  const guide = [
    ["Home", "Adaptly analyzes your saved results and recommends what you should work on next: Next Best Action, Today's Mission, Adaptly Noticed, a mastery snapshot, and your active study plan."],
    ["Planning", "Generate and review AI study plans for the official SAT and for practice tests, built from your saved scores, mistakes, and mastery."],
    ["My Results", "Record official SAT scores, practice-test scores, and mistakes under Test Scores and Mistakes. These become the evidence Adaptly uses to understand your progress. Official SAT records can contribute to your superscore; practice-test scores never do."],
    ["Practice", "Complete Adaptly-recommended adaptive practice, run a full-length timed SAT simulation, or open trusted official and vendor SAT resources."],
    ...
    ["Settings", "Open with the gear icon in the top right. Manage your profile, settings, data & privacy, help, and application information."],
  ];
  return (
    <>
      <PanelHeading title="Help & User Guide" sub="What each part of Adaptly does, in plain language." />
      {guide.map(([t, d]) => (
        <PanelCard key={t}>
...
function AboutPanel() {
  return (
    <>
      <PanelHeading title="About Adaptly" />
      <PanelCard>
        <p style={{ fontSize: 14, color: C.ink, lineHeight: 1.6, margin: 0 }}>
          Adaptly is a digital SAT planning, tracking, analytics, and study-planning prototype. It helps students
      ...
      <PanelCard>
        ...
        <p style={{ fontSize: 13.5, color: C.ink2, lineHeight: 1.6, margin: 0 }}>
          Adaptly is an independent educational prototype. The project explores how responsible data tracking,
      ...
      <PanelCard>
        ...
        <ul style={{ margin: 0, paddingLeft: 18, fontSize: 13.5, color: C.ink2, lineHeight: 1.7 }}>
          <li><b>Adaptly Agent (Home):</b> Next Best Action, Today's Mission, and Adaptly Noticed, from your real data.</li>
          ...
          <li><b>AI Study Planner:</b> agent-managed, AI-generated study plans built from your real results.</li>
      ...
      <PanelCard>
        ...
        <p style={{ fontSize: 13.5, color: C.ink2, lineHeight: 1.6, margin: 0 }}>
          Adaptly was developed by Ansh Saini, a high school student, as an independent
      ...
      <div style={{ background: C.soft, border: `1px solid ${C.line}`, borderRadius: 14, padding: 20, marginTop: 4 }}>
        ...
        {[
          ["Independent project", "Adaptly is an independent educational prototype. It is intended to help students organize test results, review mistakes, view performance trends, and create study plans."],
          ["No College Board affiliation", "Adaptly is not affiliated with, endorsed by, approved by, or sponsored by College Board or any test-preparation company. SAT® is a trademark of College Board. Adaptly does not issue official SAT scores, register students for the SAT, submit scores to colleges, reproduce official SAT questions, or provide official College Board services."],
          ["Superscore calculation", "The SAT superscore shown in Adaptly is a mathematical calculation based on official SAT scores entered by the user. Colleges and universities may have different score-use and superscoring policies. Students must verify each institution's current policy directly."],
          ...
          ["No guaranteed outcomes", "Adaptly does not guarantee score improvement, college admission, scholarship eligibility, academic performance, or any other educational outcome."],
          ...
          ["Prototype status", "Adaptly is a student-developed prototype. Features may change, and the application may contain technical errors, inaccurate outputs, or temporary interruptions."],
        ].map(([t, d]) => (
